...
file_handler = logging.FileHandler("server_iris_classifier_v1.log")
...
@app.get("/")
def read_root():
    return "Server Iris Classifier V1 is all ready to go!"
...
async def classify(iris_features: Iris):
    logger.debug(f"Incoming iris features to the Iris Classifier V1: {iris_features}")
    iris_classifier = IrisClassifierV1()
    response = JSONResponse(iris_classifier.classify_iris(iris_features))
    logger.debug(f"Outgoing classification from the Iris Classifier V1: {response}")
    return response
